      ...
      <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800 shadow-xl">
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 mb-4">
          ...
          {neynarUser?.verified_addresses?.eth_addresses.map((address) => (
            <p className="text-white text-md font-bold" key={address}>
              {truncateAddress(address)}
            </p>
          ))}

          {neynarUser?.verified_addresses?.sol_addresses.map((address) => (
            <p className="text-white text-md font-bold" key={address}>
              {truncateAddress(address)}
            </p>
          ))}
        ...
        {/* <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 space-y-2">
                    <p className="text-white text-sm"><span className="text-slate-400">Likes:</span> <span className="font-semibold ml-2">{profileData.likes}</span></p>
                    <p className="text-white text-sm"><span className="text-slate-400">Casts:</span> <span className="font-semibold ml-2">{profileData.casts}</span></p>
                    <p className="text-white text-sm"><span className="text-slate-400">Comments:</span> <span className="font-semibold ml-2">{profileData.comments}</span></p>
                    <p className="text-white text-sm"><span className="text-slate-400">Recasts:</span> <span className="font-semibold ml-2">{profileData.recasts}</span></p>
                  </div> */}
      </div>
    </div>
  );
